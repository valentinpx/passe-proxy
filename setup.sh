...
prompt_optional() {
    # prompt_optional VAR "question"  — empty input is accepted
    _var=$1 _msg=$2
    printf '%s: ' "$_msg"
    read -r _val
    eval "$_var=\"\$_val\""
...
# Write Caddyfile: snippet + AUTH_DOMAIN block + one stanza per app.
# Usage: write_caddyfile "app1.example.com:3000\napp2.example.com:8080\n"
write_caddyfile() {
    _apps=$1
    {
        cat <<'CADDY_HEADER'
(protected) {
	forward_auth oauth2-proxy:4180 {
		...
		handle_response @unauthorized {
			redir * https://{$AUTH_DOMAIN}/oauth2/sign_in?rd={scheme}://{host}{uri}
		}
	}
...
# --- apps (regenerated by setup.sh; edit freely) ---
CADDY_HEADER
        printf '%b' "$_apps" | while IFS=: read -r _d _p; do
            [ -z "$_d" ] && continue
            printf '%s {\n\timport protected %s\n}\n' "$_d" "$_p"
        done
    } > Caddyfile
...
if [ "${SKIP_CONFIG:-0}" != "1" ]; then
    ...
    prompt BASE_DOMAIN "Base domain for cookie sharing (e.g. example.com)"
    prompt ACME_EMAIL  "Email for Let's Encrypt notifications"
    prompt AUTH_PREFIX "Subdomain prefix for Pocket ID auth" "auth"
    AUTH_DOMAIN="${AUTH_PREFIX}.${BASE_DOMAIN}"

    # ── 4b. collect protected apps (at least one required) ──────────

    APPS=""
    _first_app=1
    while true; do
        if [ "$_first_app" -eq 1 ]; then
            prompt_optional APP_PREFIX "Subdomain prefix for your first protected app (empty = ${BASE_DOMAIN})"
        else
            prompt_optional APP_PREFIX "Another app prefix (empty to finish; '.' = ${BASE_DOMAIN})"
            [ -z "$APP_PREFIX" ] && break
        fi
        case "$APP_PREFIX" in
            ''|'.') _app_domain="$BASE_DOMAIN" ;;
            *)      _app_domain="${APP_PREFIX}.${BASE_DOMAIN}" ;;
        esac
        prompt APP_PORT "  upstream port for $_app_domain" "3456"
        APPS="${APPS}${_app_domain}:${APP_PORT}
"
        _first_app=0
    done
...
# Protected apps are declared in the Caddyfile — use setup.sh or edit it directly.
...
EOF
    ...
    write_caddyfile "$APPS"
    ok "Caddyfile written with $(printf '%b' "$APPS" | grep -c ':') protected app(s)"
fi
...
echo "     - Name: anything (e.g. \"passkey-proxy\")"
echo "     - Callback URL: https://${AUTH_DOMAIN}/oauth2/callback"
...
if [ "${SKIP_CONFIG:-0}" != "1" ]; then
    printf '%b' "$APPS" | while IFS=: read -r _d _p; do
        [ -z "$_d" ] && continue
        echo "  App:   https://${_d}"
    done
else
    echo "  Apps:  see Caddyfile"
fi
echo "══════════════════════════════════════════════════════"
echo ""
